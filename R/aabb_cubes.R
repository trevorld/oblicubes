...
#' @noRd
aabb_cubes <- function(df, scale = 0.5, angle = 45, ...) {
	if (nrow(df) == 0) {
		return(list(
	...
	list(x = x, y = y, z = z, x_op = x_op, y_op = y_op)
}

# z is top of cuboid (instead of center of cube)
aabb_cuboids <- function(df, scale = 0.5, angle = 45, ...) {
	if (nrow(df) == 0) {
		return(list(
			x = c(NA_real_, NA_real_),
			y = c(NA_real_, NA_real_),
	...
	df <- rbind(
		df[, c("x", "y", "z")], # top cuboid
		data.frame(x = df$x, y = df$y, z = 0)
	) # bottom cuboid
