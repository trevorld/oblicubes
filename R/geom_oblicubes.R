	...
	ggplot2::ggproto(
		...
		draw_panel = function(
			...
			if (hasName(coord, "ratio") && coord$ratio != 1) {
				stop(
					"'geom_oblicubes()' will not work correctly if not using an aspect ratio of 1."
				)
			}
		...
		setup_data = function(data, params) {
			...
			l <- aabb_cubes(data, scale = params$scale, angle = params$angle)
