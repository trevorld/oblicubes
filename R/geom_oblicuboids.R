...
create_GeomOblicuboids <- function() {
	# nocov start
	ggplot2::ggproto(
		"GeomOblicuboids",
		ggplot2::Geom,
		...
		draw_panel = function(
			...
			if (hasName(coord, "ratio") && coord$ratio != 1) {
				stop(
					"'geom_oblicuboids()' will not work correctly if not using an aspect ratio of 1."
				)
			}
		...
		setup_data = function(data, params) {
			...
			l <- aabb_cuboids(data, scale = params$scale, angle = params$angle)
